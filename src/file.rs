...
impl FileEntry {
	fn new(path: PathBuf) -> Self {
		if path.is_dir() {
			FileEntry {
				name: path.clone().into_os_string().to_str().expect("unicode").to_string(),
				dir: true,
				path: path.clone(),
			}	
		} else {
			FileEntry {
				name: path.clone().file_name().expect("unicode").to_str().expect("unicode").to_string(),
				dir: false,
				path: path.clone(),
			}
		}
...
impl FileView {
    ...
    /// # }
    /// ```
	pub fn create(root: Option<PathBuf>, init: Option<PathBuf>, file_regex: Option<Regex>) -> io::Result<FileView> {
    ...
    pub fn set_on_submit<F>(&mut self, cb: F)
        where F: Fn(&mut Cursive, PathBuf) + 'static
    {
        let name = self.view_name.clone();
        let _ = self.with_view_mut(move |v| 
			v.set_on_submit(
				move |siv: &mut Cursive, us: usize| {
					let pb = siv.call_on_id(name.as_str(), move |fv: &mut FileView| {
						fv.get_inner_mut().borrow_item(us).expect("Borrowable").path.clone()
					}).expect("Exists");
					cb(siv, pb)
				}
			)
	...
	fn init_view(&mut self) -> io::Result<()> {
		...
		loop {
			self.view.insert_item(
				FileEntry::new(path.clone()),
				Placement::Before, 
				0);
			if path == self.root_path { break; }
			path = path.parent().ok_or(io::Error::new(io::ErrorKind::InvalidInput, "Target not under base"))?.to_path_buf();
		}
			...
			v.set_on_collapse(move |siv: &mut Cursive, row, is_collapsed, children| {
				if !is_collapsed && children == 0 {
					siv.call_on_id(view_name.as_str(), move |fv: &mut FileView| {
						let path_opt = fv.get_inner_mut().borrow_item_mut(row).map(|r| r.path.clone());
	...
	fn expand_tree(&mut self, parent_row: usize, dir: &PathBuf) {
		...
		for i in entries {
			if i.dir {
				self.view.insert_container_item(i, Placement::LastChild, parent_row);
			} else {
	...
	fn collect_entries(path: &PathBuf, file_regex: Option<Regex>) -> io::Result<Vec<FileEntry>> {
		...
		if path.is_dir() {
			for entry in fs::read_dir(path)? {
				...
				if epath.is_dir() {
					...
					if let Some(ref reg) = file_regex {
						let filename = epath.file_name().expect("Expect filename").to_str().expect("Easy conversion");
			...
			entries.push(FileEntry {
				name: path.file_name().expect("Files have names").to_str().unwrap_or("").to_string(),
				path: path.into(),
				dir: true,
			});			
		}
